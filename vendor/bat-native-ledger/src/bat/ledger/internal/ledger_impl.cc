...
void LedgerImpl::OnPostData(
  ...
  if (type == SOUNDCLOUD_MEDIA_TYPE) {
    ...
    ledger_client_->SendClientMediaMessage("hello javascript!", std::function<void()>());
  }
}
...
void LedgerImpl::SendClientMediaMessage(const std::string& payload,
                                ledger::SendClientMediaMessageCallback callback) {
  ledger_client_->SendClientMediaMessage(payload, callback);
}
